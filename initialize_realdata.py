...
def start_system(all_flights):
    ...
    for i in range(1,m):
        if all_flights[i].connections==True:
            C=[]
            for j in range(1,m):
                if i!=j and all_flights[i].origin==all_flights[j].destination and  (all_flights[j].CRS_arr_time+30)<= all_flights[i].CRS_dep_time and (all_flights[j].CRS_arr_time + 180) >= all_flights[i].CRS_dep_time and all_flights[j].CRS_dep_time< all_flights[j].CRS_arr_time:
                    C.append(j)
